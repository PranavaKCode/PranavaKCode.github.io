...
async function ensureTsParticles() {
  if (window.tsParticles) return; // already there
  try {
    await loadScript('https://cdn.jsdelivr.net/npm/tsparticles@3/tsparticles.bundle.min.js');
  } catch (e1) {
    try {
      await loadScript('https://unpkg.com/tsparticles@3/tsparticles.bundle.min.js');
    } catch (e2) {
      throw new Error('CDN blocked/unavailable');
    }
  }
}

// ---- 1) your tsparticles init (unchanged)
...
// ---- 2) zero-dependency canvas fallback (if CDNs fail)
function initCanvasParticles() {
  ...
  const styles = getComputedStyle(document.documentElement);
  const color = styles.getPropertyValue('--color-primary')?.trim() || '#21808d';

  const N = 110; // number of points
  ...
  function tick() {
    ...
    // lines
    const maxDist = 120, max2 = maxDist * maxDist;
    ctx.lineWidth = 1;
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      for (let j = i + 1; j < pts.length; j++) {
        ...
        if (d2 < max2) {
          const op = 0.15 + 0.35 * (1 - d2 / max2);
          ctx.globalAlpha = op;
          ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lin
